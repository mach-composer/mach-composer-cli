...
resource "aws_apigatewayv2_stage" "{{ endpoint.Key|slugify }}_default" {
  ...
  {% if endpoint.AWS.ThrottlingRateLimit or endpoint.AWS.ThrottlingBurstLimit -%}
  default_route_settings {
    {% if endpoint.AWS.ThrottlingBurstLimit %}
    throttling_burst_limit = {{ endpoint.AWS.ThrottlingBurstLimit }}
    {%- endif -%}
    {% if endpoint.AWS.ThrottlingRateLimit %}
    throttling_rate_limit = {{ endpoint.AWS.ThrottlingRateLimit }}
    {% endif %}
  }{% endif %}
